...
- Put shared pilot VB.NET source flat under
  ...
  `AccessManager*` filename prefixes. Do not nest App_Code subfolders under
  `AdminShell` (ASP.NET compiles them as separate assemblies and breaks
  cross-references; that shows up as remote 500s on `login.ashx`).
...
- Sign in as `dhoffman` and open all five navigation tabs (Views, logs, Access Manager).
...
`/admin/admin/cgi-bin/...`. To remove the entire pilot, remove `dev\adminshell`
