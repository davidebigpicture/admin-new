...
$pilotConfigPath = Join-Path $PSScriptRoot "..\..\web.config"
[xml]$pilotConfig = Get-Content -LiteralPath $pilotConfigPath
$globalAdminRoot = ($pilotConfig.configuration.appSettings.add | Where-Object { $_.key -eq "GlobalAdminRootPath" }).value.TrimEnd("/")
$grantCapability = ($pilotConfig.configuration.appSettings.add | Where-Object { $_.key -eq "AccessManagerGrantCapability" }).value.TrimStart("/")
$canonicalPath = "/" + ($globalAdminRoot.TrimStart("/") + "/" + $grantCapability).Replace("//", "/")
...
"@
    ...
    if ($accessCount -lt 1) {
        throw "The configured pilot user does not have the Access Manager grant capability ACL ($canonicalPath)."
    }

    Write-Output "Access Manager data smoke test passed for $canonicalPath."
}
finally {
    $connection.Dispose()
}
